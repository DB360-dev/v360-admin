...
import { ORDER_VIEWS } from "@/lib/status";
...
export function Orders() {
  ...
  const view = ORDER_VIEWS.find((v) => v.key === viewKey) ?? ORDER_VIEWS[0];
  ...
  const q = useOrderList({ statuses: view.statuses, brandId: brandId || undefined, search, from, to, page });
  const rows = q.data?.rows ?? [];
  const total = q.data?.total ?? 0;
  const pages = Math.max(1, Math.ceil(total / PAGE_SIZE));
  const filters = !!(search || from || to || brandId);
  const count = (ss: typeof view.statuses) => counts ? (ss ?? Object.keys(counts) as (keyof typeof counts)[]).reduce((n, s) => n + (counts[s] ?? 0), 0) : null;
    ...
    <>
      ...
      <div role="tablist" className="-mx-1 mb-4 flex gap-1 overflow-x-auto border-b border-line px-1">
        {ORDER_VIEWS.map((v) => {
          ...
          return (
            <button key={v.key} role="tab" aria-selected={v.key === view.key} onClick={() => update({ view: v.key === "all" ? null : v.key })}
              className={`-mb-px flex items-center gap-1.5 whitespace-nowrap border-b-2 px-3 py-2 text-[13.5px] ${v.key === view.key ? "border-primary font-medium" : "border-transparent text-muted hover:text-ink"}`}>
      ...
      <div className="panel overflow-hidden">
        <div className="overflow-x-auto">
          <table className="w-full min-w-[960px] text-[13.5px]">
            <thead className="table-head"><tr><th>Order</th><th>Brand</th><th>Date</th><th>Customer</th><th className="text-right">COD</th><th>Status</th><th>Where</th></tr></thead>
              ...
              <tbody className={`table-body ${q.isFetching ? "opacity-70" : ""}`}>
                {rows.map((o) => (
                  <tr key={o.id} onClick={() => navigate(`/orders/${o.id}`)} className="cursor-pointer hover:bg-sunken/50">
                    ...
                    <td><div className="flex items-center gap-2"><StatusBadge status={o.status} /><span className="text-[12px] text-faint">{since(o.status_changed_at)}</span></div></td>
                    <td className="max-w-[200px] truncate text-muted">{where(o)}</td>
                  </tr>
                ))}
              </tbody>
            )}
