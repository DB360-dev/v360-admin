-- Per-item weight capture at hub receiving
--
-- The receiving dialog now records a weight next to each item line. The
-- operator enters one item's own weight per line and the frontend sums
-- (item weight x quantity) into a single order total, which replaces the
-- old "one chosen item weight x total quantity" shortcut. receive_order
-- therefore stores p_order_weight_kg verbatim in order_freight_weights;
-- the signature stays (uuid, numeric, jsonb, text) so the earlier grant of
-- receive_order(uuid, numeric, jsonb, text) to authenticated still applies.
